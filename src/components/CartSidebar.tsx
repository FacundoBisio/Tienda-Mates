    ...
    <>
      ...
      >
        <div className="flex justify-between items-center px-6 py-5 border-b border-[#E8E3DC]">
          ...
          <button onClick={onClose} className="text-[#888] hover:text-[#1C1C1C] transition-colors text-xl leading-none" aria-label="Cerrar carrito">
            ✕
          </button>
        </div>
              ...
              {cartItems.map((item) => (
                <li key={item.id} className="flex gap-4 items-start">
                  ...
                  <div className="flex-grow">
                    ...
                    <p className="text-xs text-[#888] mt-0.5">${(parseFloat(item.price) * item.quantity).toLocaleString('es-AR')}</p>
                    <div className="flex items-center gap-3 mt-3">
                      <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-7 h-7 rounded-full border border-[#E8E3DC] flex items-center justify-center text-sm text-[#1C1C1C] hover:border-[#4C674A] transition-colors">−</button>
                      <span className="text-sm w-4 text-center">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="w-7 h-7 rounded-full border border-[#E8E3DC] flex items-center justify-center text-sm text-[#1C1C1C] hover:border-[#4C674A] transition-colors">+</button>
                    </div>
                  </div>
                  <button onClick={() => removeFromCart(item.id)} className="text-[#BBBBB0] hover:text-red-400 transition-colors text-sm mt-1 flex-shrink-0" aria-label="Eliminar">✕</button>
                </li>
              ))}
            </ul>
